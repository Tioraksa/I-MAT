...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <!-- Button Tambah FAS -->
            <button">
                <a href="<?= base_url('admin/material_predictions'); ?>" class="btn btn-primary mb-3">Kebutuhan Material</a>
            </button>



            <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#addFasModal">
                + Tambah FAS
            </button>

            <!-- Filter Tanggal -->
            <div class="d-flex justify-content-between mb-3">
                <a href="<?= base_url('admin/fas?date=' . $prev_date); ?>" class="btn btn-outline-primary">← Sebelumnya</a>

                <form method="GET" action="<?= base_url('admin/fas'); ?>">
                    <input type="date" name="date" value="<?= $selected_date; ?>" class="form-control d-inline-block" style="width: 200px;">
                    <button type="submit" class="btn btn-primary">Tampilkan</button>
                </form>

                <a href="<?= base_url('admin/fas?date=' . $next_date); ?>" class="btn btn-outline-primary">Selanjutnya →</a>
            </div>
...
<!-- Modal Tambah FAS -->
<div class="modal fade" id="addFasModal" tabindex="-1" role="dialog" aria-labelledby="addFasModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addFasModalLabel">Tambah FAS</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('admin/add_fas'); ?>" method="POST">
                    <div class="form-group">
                        <label>Warna Lot</label>
                        <select name="warna_lot" class="form-control">
                    ...
                    <div class="form-group">
                        ...
                        <select name="model" id="modelSelect" class="form-control">
                            ...
                            <option value="BUA1A9-010A">BUA1A9-010A</option>
                            <option value="BUA1C2-010A">BUA1C2-010A</option>
                            <option value="BUA1C3-010A">BUA1C3-010A</option>
                            <option value="BWE1E3-010A">BWE1E3-010A</option>
                            <option value="BRW6C4-010A">BRW6C4-010A</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Invoice</label>
                        <input type="text" id="invoiceInput" name="invoice" class="form-control" readonly>
                    </div>

                    
                    <div class="form-group">
                        <label>CKD Set Name</label>
                        <input type="text" id="ckdSetInput" name="ckd_set_name" class="form-control" readonly>
                    </div>
...
$(document).ready(function(){
    $(".show-material").click(function(){
        ...
        $.ajax({
            url: "<?= base_url('admin/get_materials'); ?>",
            method: "GET",
            data: { model: model },
            dataType: "json",
...
$(document).ready(function(){
    $("#modelSelect").change(function(){
        ...
        $.ajax({
            url: "<?= base_url('admin/get_material_data'); ?>",
            method: "GET",
            data: { model: model },
            dataType: "json",
            success: function(data) {
